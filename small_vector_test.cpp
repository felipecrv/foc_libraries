...
class Constructable {
 ...
 public:
  ...
  /*implicit*/ Constructable(int val) : constructed(true), value(val) {
    ++numConstructorCalls;
  }
...
TYPED_TEST(SmallVectorTest, PushPopTest) {
  ...
  if (!RequiresGrowth) {
    ...
    EXPECT_EQ(Constructable::numConstructorCalls,
...
TYPED_TEST(SmallVectorTest, MoveAssignTest) {
  ...
  EXPECT_EQ(Constructable::numConstructorCalls-2,
            Constructable::getNumDestructorCalls());
...
TYPED_TEST(DualSmallVectorsTest, MoveAssignment) {
  ...
  // are two live objects after clearing the other vector.
  this->otherVector.clear();
  EXPECT_EQ(Constructable::numConstructorCalls-4,
            Constructable::getNumDestructorCalls());
