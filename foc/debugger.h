...
// ## [0.0.1] - 2019-07-30
//
// - Get the code from Chromium's base library
// - Drop VerifyDebugger()
// - Include Alias which comes from a different file in Chromium

#pragma once

#include "support.h"

#ifdef FOC_DEBUGGER_IMPLEMENTATION
...
#if defined(FOC_OS_MACOSX)
#include <AvailabilityMacros.h>
#endif

#if defined(FOC_OS_MACOSX) || defined(FOC_OS_BSD)
#include <sys/sysctl.h>
#endif

#endif  // FOC_DEBUGGER_IMPLEMENTATION
...
//   FOC_DEBUG_ALIAS_FOR_CSTR(name_copy, p->name, 16);
//   CHECK(false);
...
#define FOC_DEBUG_ALIAS_FOR_CSTR(var_name, c_str, char_count) \
...
#if defined(FOC_OS_MACOS) || defined(FOC_OS_BSD)
...
# endif
  ...
  int sysctl_result = sysctl(mib, sizeof(mib) / sizeof(mib[0]), &info, &info_size, NULL, 0);
  ...
  is_set = true;
# if defined(FOC_OS_FREEBSD)
  being_debugged = (info.ki_flag & P_TRACED) != 0;
# elif defined(FOC_OS_BSD)
...
#elif defined(FOC_OS_LINUX) || defined(FOC_OS_ANDROID) || defined(FOC_OS_AIX)
...
#if defined(FOC_ARCH_CPU_ARMEL)
# define DEBUG_BREAK_ASM() asm("bkpt 0")
#elif defined(FOC_ARCH_CPU_ARM64)
# define DEBUG_BREAK_ASM() asm("brk 0")
#elif defined(FOC_ARCH_CPU_MIPS_FAMILY)
# define DEBUG_BREAK_ASM() asm("break 2")
#elif defined(FOC_ARCH_CPU_X86_FAMILY)
# define DEBUG_BREAK_ASM() asm("int3")
#endif

#if defined(NDEBUG) && !defined(FOC_OS_MACOS) && !defined(FOC_OS_ANDROID)
...
void BreakDebugger() {
  ...
  DEBUG_BREAK();
#if defined(FOC_OS_ANDROID) && !defined(OFFICIAL_BUILD)
...
FOC_ATTRIBUTE_NOINLINE void Alias(const void* var) {}
